...
public class JwtConverterUnitTest {
    ...
    public void testConvertAuthorities() {
        ...
        var claims = Map.of(
                "realm_access", Map.of("roles", List.of("realm_access")),
                "resource_access", Map.of("client", Map.of("roles", List.of("resource_access"))),
        ...
        assertNotNull(convertedJwt);
        var actualRoles = convertedJwt.getAuthorities().stream().map(GrantedAuthority::getAuthority).toList();
        assertTrue(
                expectedRoles.containsAll(actualRoles),
